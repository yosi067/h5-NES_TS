    ...
    var createModule2 = (() => {
      ...
      return function(createModule3) {
        ...
        var ASM_CONSTS = { 1718756: () => {
          ...
          const stop = function() {
            if (Module2.n64AudioWorkletPort) {
              Module2.n64AudioWorkletPort.postMessage({ type: "state", running: false, muted: true });
              Module2.n64AudioWorkletPort.close();
            }
            if (Module2.n64AudioWorkletNode) {
              Module2.n64AudioWorkletNode.disconnect();
              Module2.n64AudioWorkletNode = void 0;
            }
            if (Module2.n64AudioWorkletSilentGain) {
              Module2.n64AudioWorkletSilentGain.disconnect();
          ...
          const configureAudioWorklet = function(workletUrl) {
            if (Module2.n64AudioWorkletNode) return Promise.resolve(true);
            if (Module2.n64AudioWorkletPromise) return Module2.n64AudioWorkletPromise;
            Module2.n64AudioWorkletPromise = new Promise(function(resolve) {
              let attempts = 0;
              const waitForAudio = function() {
                const SDL2 = Module2.SDL2;
                ...
                SDL2.audioContext.audioWorklet.addModule(workletUrl).then(function() {
                  if (Module2.n64AudioWorkletNode) return true;
                  if (!Module2.SDL2 || !Module2.SDL2.audio || !Module2.SDL2.audio.scriptProcessorNode) {
                    return false;
                  }
                  const scriptProcessor = Module2.SDL2.audio.scriptProcessorNode;
                  const workletNode = new AudioWorkletNode(SDL2.audioContext, "n64-audio-processor", { numberOfInputs: 0, numberOfOutputs: 1, outputChannelCount: [2] });
                  ...
                  Module2.n64AudioWorkletNode = workletNode;
                  Module2.n64AudioWorkletSilentGain = silentGain;
                  Module2.n64AudioWorkletPort = workletNode.port;
                  return true;
            ...
            return Module2.n64AudioWorkletPromise;
          };
          const resumeAudio = function() {
            const SDL2 = Module2.SDL2;
            const audioSuspended = SDL2 && SDL2.audioContext && SDL2.audioContext.state !== "running";
            if (audioSuspended && Module2.n64AudioWorkletPort) {
              Module2.n64AudioWorkletPort.postMessage({ type: "clear" });
            }
            if (audioSuspended) {
        ...
        function audio_web_push_samples(stream, len) {
          if (!Module2.n64AudioWorkletPort || len <= 0) return;
          const sampleCount = len >> 1;
          const samples = new Float32Array(sampleCount);
          const source = stream >> 1;
          for (let index = 0; index < sampleCount; ++index) samples[index] = HEAP16[source + index] / 32768;
          Module2.n64AudioWorkletPort.postMessage({ type: "samples", samples }, [samples.buffer]);
        }
        ...
        function _endStats(numberOfRecompiles, rspMs, dlistMs, rdpMs, presentMs, audioMs, triangleDrawMs, rectDrawMs, trianglePrepareMs, triangleUploadMs, triangleSubmitMs, triangleRestoreMs, triangleDrawCalls, rectDrawCalls, audioUnderruns, audioCallbackCount, audioPartialUnderruns, audioEmptyUnderruns, audioMaxCallbackGapMs) {
          if (Module2.endStats) {
            Module2.endStats(numberOfRecompiles, rspMs, dlistMs, rdpMs, presentMs, audioMs, triangleDrawMs, rectDrawMs, trianglePrepareMs, triangleUploadMs, triangleSubmitMs, triangleRestoreMs, triangleDrawCalls, rectDrawCalls, audioUnderruns, audioCallbackCount, audioPartialUnderruns, audioEmptyUnderruns, audioMaxCallbackGapMs);
          }
        }
