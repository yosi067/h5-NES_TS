...
**GitHub Pages部署**：rebuilt runtime納入版本化artifact。Vite production build會在bundle/Wasm/data/manifest缺少，或manifest不是64 MiB initial memory時直接失敗。Pages CDN實測會忽略同名資產的version query，曾把新JS/Wasm與舊data混用；fork的main bundle、data與Wasm因此發布為帶相同asset version的實體檔名。現有GitHub Actions依repository name設定base URL；正常手機不帶query時載入fork與triangle streaming，維持320x240 backing與完整4:3畫面。
