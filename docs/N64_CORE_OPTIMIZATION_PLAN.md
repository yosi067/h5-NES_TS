...
- triangle stream已升為正常mobile profile的production路徑：手機不需query string即自動使用rebuilt fork與`PersistentBuffers=1`，rectangle維持關閉；desktop仍使用npm runtime，手機可用`?n64Runtime=npm`緊急回退。
...
**GitHub Pages部署**：rebuilt runtime已納入版本化artifact，Vite production build會在bundle/Wasm/data缺少時直接失敗。現有GitHub Actions依repository name設定base URL；本機以`/h5-NES_TS/` production preview及iPhone user agent驗證，不帶query即可載入rebuilt fork。Super Mario 64與Mario Kart 64均維持320x240 backing和完整4:3畫面，切換至NES後亦正確還原256x240 canvas。
...
- 2026-07-19本機驗收：12項N64 focused tests、patch bootstrap、固定Emscripten N64 build與完整production build通過。Chrome/RTX 3080 Ti以fork、stream ring啟動Super Mario 64並進入3D開場，無page/runtime error；短窗49.7至56.1 VI/s、triangle draw約0.1至0.2 ms/VI，暖機後audio underruns為每5秒0至2次。桌面GPU結果只證明可遊玩與telemetry有效，不作為iPhone效能結論。
...
- `?n64Runtime=fork` 明確啟用 rebuilt artifact；未帶參數時仍使用 npm runtime 作為可運作的 rollback 路徑。
