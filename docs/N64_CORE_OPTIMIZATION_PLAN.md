...
- triangle stream的iPhone效能成果保留，但rebuilt fork在2026-07-19 production驗收發現可重現的Asyncify rewind `memory access out of bounds`，因此暫停作為正式路徑。手機與desktop目前都預設使用npm runtime；fork與`PersistentBuffers=1`只由`?n64Runtime=fork`或benchmark明確啟用，待修復後重新通過三款遊戲驗收才能升回production。
...
**GitHub Pages部署**：rebuilt runtime仍納入版本化artifact供隔離測試，Vite production build會在bundle/Wasm/data缺少時直接失敗。現有GitHub Actions依repository name設定base URL；正常手機不帶query時載入npm runtime，維持320x240 backing與完整4:3畫面。只有明確指定fork時才載入實驗artifact。
...
- 2026-07-19重新驗收正式artifact：`?n64Runtime=fork`在Chrome與iPhone均可於`start()`的Asyncify rewind重現Wasm `memory access out of bounds`，先前本機成功不能代表目前提交的artifact。正常mobile production因此回退npm runtime；triangle ring保留為實驗，不再宣稱已正式上線。
