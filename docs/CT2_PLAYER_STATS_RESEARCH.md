...
- 遊戲清單「足球小將 2 超級前鋒」指向 ZIP；實際第一個可遊玩檔案為下述已驗證 header alias。伺服器及本機 ZIP 載入都解壓後，以內部檔名／原始 bytes 進入同一 `startGame()` 原生路徑；不是以 ZIP 檔名授權功能。
- `Emulator::load_rom()` 在 reset／第一幀前依完整 SHA 與 mapper 啟用 64；`EmuWasm` 與 `NesWasm` 都適用。沒有 DEV、裝置型號、localStorage 或 UI 開關的預設依賴。翻譯資產下載／驗證失敗仍載入原 ROM，因此不會失去此預設。現有支援啟動路徑未發現缺少預設，無須額外寫入 RAM 或重複在 UI 啟用。
- 可承諾的是「載入此版正式程式、且能成功執行原生 WASM 的裝置，開啟上述驗證 ROM 預設自然滿級」。不能承諾所有裝置／瀏覽器必定能執行；本次沒有實機觸控驗收。原生核心未初始化時顯示錯誤，不會自動因裝置類型改走 FCEUmm；原生 NES ROM 載入回傳失敗或拋錯才走 FCEUmm，該 fallback **沒有**能力調整。
- 舊 BPS 修改版、未驗證漢化／修改 ROM 不支援；相同名稱不構成支援。正式部署需包含本次重建的 JS 與 WASM，仍開著舊版本分頁不算已取得新版。
- 本輪補強 ZIP 第一個可遊玩 entry 與兩個 WASM 包裝器測試：無 DOM／翻譯資產／storage，初次載入為 64，先關閉或改為 1／32 再重新載入也恢復 64。`npm run build`、啟用原 ROM 的 `test:ct2:stats`（5 Node + 8 UI／預覽）及 `test:ct2:stats:rom`（2 原 ROM 測試）均通過；原 routine 每組 1,472 計算、runtime 開機體力 976 與 33 個換位情境再次驗證。未執行 commit／merge／push。
