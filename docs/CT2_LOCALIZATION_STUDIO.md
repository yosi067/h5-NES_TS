...
目前原生 NES 改為完整硬體深拷貝（CPU、PPU、APU、匯流排、mapper／卡帶、手把、時鐘與 DMC DMA）的**同核心、本次執行暫存**。保留最近 16 次快照；未知、舊格式、過期或重新載入前的 token 會先拒絕，不修改執行中狀態。讀取成功後清空未播放的未來音訊並更新畫面。
...
- `node --test tools/nes-temporary-state.test.mjs`：前端六項回歸。
